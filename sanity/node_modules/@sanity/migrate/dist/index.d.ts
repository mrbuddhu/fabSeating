...
declare const ALLOWED_PROPERTIES: readonly ["fetch", "clone", "config", "withConfig", "getDocument", "getDocuments", "users", "projects"];

declare type AllowedMethods = (typeof ALLOWED_PROPERTIES)[number];

declare type AnyArray<T = any> = readonly T[] | T[];
...
declare type NormalizeReadOnlyArray<T> = T extends readonly [infer NP, ...infer Rest] ? [NP, ...Rest] : T extends readonly (infer NP)[] ? NP[] : T;
...
declare type Tuplify<T> = T extends readonly [infer NP, ...infer Rest] ? [NP, ...Rest] : T extends readonly (infer NP)[] ? NP[] : [T];
